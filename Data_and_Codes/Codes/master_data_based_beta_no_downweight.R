...
# keep only NMA estimates for the SMD's
basic_comp_CA = basic_comp_CA %>%
  filter(!grepl("pred", ind))
...
basic_comp_CA$model_type =
  rep("Data based beta-No DW", nrow(basic_comp_CA))
...
# add column to specify the type of the approach used
SucrasCA$model_type =
  rep("Data based beta-No DW", nrow(SucrasCA))
...
write.csv(basic_comp_CA,
          "Results\\data_based_beta_No_DW.csv",
          row.names = F)

# These results are part of the "SUCRAS.xlsx" file in the "Intermediate results" folder
write.csv(SucrasCA,
          "Results\\SUCRA_data_based_beta_No_DW.csv",
          row.names = F)

write.csv(league_table,
          "Results\\Supporting_table_11.csv",
          row.names = F)
